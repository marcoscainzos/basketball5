...
import { createDailyGame, DailyGame, Difficulty, GameMode, getDateKey, getPlayer, nextChallenger, nextPair, STAT_META } from "@/lib/game";
...
export default function OneVsOneGame() {
  ...
  useEffect(() => { if (saved && leagueContext?.modeId && keys.includes(leagueContext.modeId as ChallengeKey) && !active) openChallenge(leagueContext.modeId as ChallengeKey); }, [saved, leagueContext, active]);
  ...
  function openChallenge(key: ChallengeKey) { if (!saved || (!leagueContext && saved[key].attemptsUsed >= 2)) return; const existing = saved[key]; const config = details(key); if (!existing.game || existing.game.status !== "playing") { const fresh = createDailyGame(getDateKey(), config.mode, existing.attemptsUsed, config.difficulty); fresh.best = existing.best; setSaved({ ...saved, [key]: { ...existing, game: fresh } }); } setActive(key); setSelectedId(null); setFeedback(""); }
  function finish(updated: DailyGame) { if (!active || !saved) return; const previous = saved[active]; setSaved({ ...saved, [active]: { ...previous, attemptsUsed: Math.min(leagueContext ? 1 : 2, previous.attemptsUsed + 1), best: Math.max(previous.best, updated.score), game: updated } }); if (leagueContext) recordLeagueResult(leagueContext,{rawScore:updated.score,maxRawScore:Math.max(updated.score,1),outcome:"completed"}).then(setLeagueResult); }
  function backFromLeague() { if (leagueContext) window.location.href = `/leagues/${leagueContext.leagueCode}`; else setActive(null); }
  function choose(playerId: number) { if (!game || !current || !challenger || !active || !saved || game.status !== "playing" || selectedId !== null) return; const winner = current[game.stat] > challenger[game.stat] ? current : challenger; const correct = playerId === winner.id; setSelectedId(playerId); setFeedback(correct ? `${winner.name} · ${winner.season} · ${winner[game.stat].toFixed(1)}` : `${lang === "es" ? "No. Era" : "No. It was"} ${winner.name} (${winner.season}) ${lang === "es" ? "con" : "with"} ${winner[game.stat].toFixed(1)}.`); setTimeout(() => { if (!correct) { finish({ ...game, best: Math.max(game.best, game.score), status: "lost" }); return; }
    ...
    const survivorLeft = refreshBoth || ((game.score + winner.id) % 2 === 0);
    const updated: DailyGame = { ...game, currentId: pair?.current.id ?? (survivorLeft ? winner.id : next!.id), challengerId: pair?.challenger.id ?? (survivorLeft ? next!.id : winner.id), usedIds: [...game.usedIds, ...newIds], revealedIds: refreshBoth ? [] : [winner.id], score: game.score + 1, best: Math.max(game.best, game.score + 1), survivorWins: refreshBoth ? 0 : survivorWins };
  ...
  if (!saved) return <main className="game-loading">{t("openingGym")}</main>;
  if (!active || !modeSave || !game || !current || !challenger) return <main className="mode-shell">
    ...
    {!showCurrent && <section className="mode-intro"><div className="mode-copy"><h1>1VS1</h1><p>{t("oneVsOneIntro")}</p></div><div className="reset-inline"><small>{t("reset")}</small><b>{countdown}</b></div></section>}
    {!showCurrent ? <section className="mode-select mode-select-two">
      <button className={`mode-card mode-historical ${saved.historical.attemptsUsed >= 2 ? "locked" : ""}`} disabled={saved.historical.attemptsUsed >= 2} onClick={() => openChallenge("historical")}><h2>{t("historical")}</h2></button>
      <button className="mode-card mode-current" onClick={() => setShowCurrent(true)}><h2>{t("current")}</h2></button>
    </section> : <div className="level-screen"><section className="level-frame"><div className="mode-select">{difficulties.map((difficulty) => { const key = `current_${difficulty}` as ChallengeKey; const item = saved[key]; const locked = item.attemptsUsed >= 2; return <button type="button" key={key} disabled={locked} className={`mode-card mode-${difficulty} ${locked ? "locked" : ""}`} onClick={() => openChallenge(key)}><span className="attempts-pill">{locked ? t("locked") : `${2-item.attemptsUsed} ${2-item.attemptsUsed === 1 ? t("attempt") : t("attempts")}`}</span><h2>{t(difficulty).toUpperCase()}</h2><i aria-hidden="true">→</i></button>; })}</div></section></div>}
  </main>;

  const winnerId = current[game.stat] > challenger[game.stat] ? current.id : challenger.id; const finished = game.status !== "playing"; const difficultyLabel = game.difficulty ? t(game.difficulty) : ""; const label = active === "historical" ? t("historical") : `${t("current")} · ${difficultyLabel}`;
  return <main className="game-shell"><nav className="site-nav"><button type="button" className="back-button" onClick={backFromLeague}>← {leagueContext ? "LIGA" : t("modes")}</button><SiteBrand link={false} /><div className="live-reset">{t("reset")} {countdown}</div></nav><header className="match-head"><div><span>{label.toUpperCase()} · {t("try")} {modeSave.attemptsUsed+1}/{leagueContext?1:2}</span><h1>{t("whoHasMore")} <b>{STAT_META[game.stat].short}?</b></h1></div><div className="match-score"><span>{t("streak")}</span><b>{game.score}</b><i/><span>{t("best")}</span><b>{Math.max(game.best,modeSave.best)}</b></div></header>{leagueResult&&<div className="league-result-pill"><span>LIGA</span><b>+{leagueResult.points}</b><small>{leagueResult.points} PTS</small></div>}<section className="duel-area"><div className={`duel-cards ${refreshingPair ? "refresh-pair" : ""}`}><PlayerCard player={current} hiddenStat={game.stat} onClick={() => choose(current.id)} disabled={finished||selectedId!==null} revealed={game.revealedIds.includes(current.id)||selectedId!==null||finished} result={selectedId!==null?(current.id===winnerId?"winner":"loser"):undefined}/><div className="vs-chip">VS</div><PlayerCard player={challenger} hiddenStat={game.stat} onClick={() => choose(challenger.id)} disabled={finished||selectedId!==null} revealed={game.revealedIds.includes(challenger.id)||selectedId!==null||finished} result={selectedId!==null?(challenger.id===winnerId?"winner":"loser"):undefined}/></div><p className="game-feedback">{feedback}</p>{finished&&<div className="finished"><b>{t("runOver")}</b><h2>{game.score} {t("inARow")}</h2><button onClick={backFromLeague}>{leagueContext ? "VOLVER A LA LIGA" : modeSave.attemptsUsed>=2?`${t("modes")} →`:t("useNextTry")}</button></div>}</section></main>;
}
