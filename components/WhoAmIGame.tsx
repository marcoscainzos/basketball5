...
type Teammate={name:string;imageUrl:string};
type Challenge={name:string;imageUrl:string;teammates:Teammate[];teams:string[]};
...
export default function WhoAmIGame(){
  ...
  function guess(name:string){if(!challenge||!save||save.status!=="playing")return;setQuery("");if(clean(name)===clean(challenge.name)){setSave({...save,guesses:[...save.guesses,name],status:"won"});return;}if(save.revealed>=5){setSave({...save,guesses:[...save.guesses,name],status:"surrendered",revealed:5});return;}setSave({...save,guesses:[...save.guesses,name],revealed:save.revealed+1});}
  ...
  return <main className="who-shell"><nav className="site-nav draft-top-nav"><button className="back-button" onClick={()=>{setMode(null);setSave(null);}}>← MODES</button><div className="wordmark"><span className="mark">CI</span><b>COURT INSIDE</b></div><span className="live-reset">RESET {countdown}</span></nav><section className="who-game"><header><span>{mode==="teammates"?"COMPAÑEROS":"TRAYECTORIA"}</span><h1>WHO AM I?</h1><b>{save.revealed}/5</b></header><div className={`who-clues ${mode}`}>{clues.slice(0,5).map((clue,index)=>{const teammate=typeof clue==="string"?null:clue;const label=typeof clue==="string"?clue:clue.name;return <div className={index<save.revealed?"visible":"hidden"} key={`${label}-${index}`}><strong>{String(index+1).padStart(2,"0")}</strong>{index<save.revealed?<>{teammate&&<img src={teammate.imageUrl} alt=""/>}<span>{label}</span></>:<span>?</span>}</div>;})}</div>{finished?<div className="who-answer"><img src={challenge.imageUrl} alt=""/><div><span>{save.status==="won"?"CORRECTO":"ERA"}</span><h2>{challenge.name}</h2><p>{save.guesses.length} {save.guesses.length===1?"intento":"intentos"}</p></div></div>:<div className="who-search"><label htmlFor="who-player">BUSCA UN JUGADOR</label><div><input id="who-player" value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&suggestions[0])guess(suggestions[0]);}} placeholder="Escribe un nombre…"/><button disabled={!suggestions[0]} onClick={()=>suggestions[0]&&guess(suggestions[0])}>PROBAR</button><button className="who-flag" aria-label="Rendirse" title="Rendirse" onClick={()=>setSave({...save,status:"surrendered",revealed:5})}>⚑</button></div>{suggestions.length>0&&<aside>{suggestions.map(name=><button key={name} onClick={()=>guess(name)}>{name}<span>＋</span></button>)}</aside>}</div>}</section></main>;
}
