...
export default function WhoAmIGame(){
  ...
  useEffect(()=>{const context=getLeagueContext(); if(context?.modeId&&(context.modeId==="teammates"||context.modeId==="journey")&&!mode) openMode(context.modeId);},[mode]);
  ...
  useEffect(()=>{const context=getLeagueContext(); if(!context||!save||!challenge||save.status==="playing"||leagueResult)return; const rawScore=save.status==="won"?Math.max(1,clues.length-(save.revealed-1)):Math.max(0,clues.length-(save.revealed-1)-1); recordLeagueResult(context,{rawScore,maxRawScore:clues.length,outcome:save.status==="won"?"won":"surrendered"}).then(setLeagueResult);},[save,challenge,clues.length,leagueResult]);
