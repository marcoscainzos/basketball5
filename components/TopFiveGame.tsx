...
export default function TopFiveGame() {
  ...
  useEffect(()=>{ const timer=setTimeout(()=>{ try { const saved=JSON.parse(localStorage.getItem(`court-inside-top5-${dateKey()}`)??"[]") as string[]; setGuessed(saved.filter((name)=>challenge.answers.some((answer)=>answer.name===name))); } catch {} setReady(true); },0); return ()=>clearTimeout(timer); },[challenge]);
  ...
  const completed=guessed.length===5;
  ...
  return <section className="top5-game">
    ...
    <div className="top5-board">{challenge.answers.map((answer,index)=>{ const found=guessed.includes(answer.name); return <article className={found?"found":""} key={answer.name}><strong>0{index+1}</strong><div>{found?<><b>{answer.name}</b><span>{answer.value.toLocaleString("es-ES")} {challenge.unit}</span></>:<b>?</b>}</div></article>; })}</div>
    <div className="player-search"><label htmlFor="top5-player">BUSCA UN JUGADOR</label><div className="search-box"><input id="top5-player" autoComplete="off" disabled={completed} value={query} onChange={(event)=>{setQuery(event.target.value);setMessage("");}} onKeyDown={(event)=>{if(event.key==="Enter"&&suggestions[0])submit(suggestions[0]);}} placeholder={completed?"RANKING COMPLETADO":"Escribe un nombre…"}/><button type="button" disabled={!suggestions[0]||completed} onClick={()=>suggestions[0]&&submit(suggestions[0])}>AÑADIR</button></div>{suggestions.length>0&&<div className="search-options">{suggestions.map((name)=><button type="button" key={name} onClick={()=>submit(name)}>{name}<span>＋</span></button>)}</div>}<p className={completed?"success":""}>{message}</p></div>
  </section>;
}
