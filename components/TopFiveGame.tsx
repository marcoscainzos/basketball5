...
export default function TopFiveGame() {
  ...
  useEffect(()=>{ const context=getLeagueContext(); if(!context||!completed||leagueResult)return; recordLeagueResult(context,{rawScore:surrendered?0:guessed.length,maxRawScore:5,outcome:surrendered?"surrendered":"won"}).then(setLeagueResult); },[completed,surrendered,guessed.length,leagueResult]);
